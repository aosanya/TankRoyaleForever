...
func blinkSmallerAction(speed : Double = 1) -> SKAction{
    let action1 = SKAction.scale(to: 0.95, duration: 0.1 * speed)
    let action2 = SKAction.scale(to: 1, duration: 0.15 * speed)
    let action3 = SKAction.sequence([action1,action2])
    return action3
}

func blinkLargerAction(speed : Double = 1) -> SKAction{
    let action1 = SKAction.scale(to: 1.1, duration: 0.1 * speed)
